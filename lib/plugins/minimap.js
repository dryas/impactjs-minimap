...
.defines(function() {
	...
	drawMiniMap: function(name, posx, posy, entities, showviewport, viewportcolor)
	{
		...
		if (showviewport)
		{
			...
			var xa = ((this.maps[name + '_offsetX'] * s) + (this.maps[name].width - (this.maps[name + '_offsetX'] * s * 2)) * (map.scroll.x / ig.game.collisionMap.pxWidth)) + 0.5;
			var ya = ((this.maps[name + '_offsetY'] * s) + (this.maps[name].height - (this.maps[name + '_offsetY'] * s * 2)) * (map.scroll.y / ig.game.collisionMap.pxHeight)) + 0.5;
